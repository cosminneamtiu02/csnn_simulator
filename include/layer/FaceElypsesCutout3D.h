#ifndef _FACE_ELYPSES_CUTOUT3D_H
#define _FACE_ELYPSES_CUTOUT3D_H
...
{
	/**
	 * @brief FaceElypsesCutout3D is a type of filtering applied on an input image, extracts certain features depending on a number of filters.
	 * the features can be low level (curve, color, etc...) or more sophisticated features(eyes, nose, etc...)
	 */
	class FaceElypsesCutout3D;

	namespace _priv
	{

#ifdef SMID_AVX256
		class FaceElypsesCutout3DImpl
		{

		public:
			FaceElypsesCutout3DImpl(FaceElypsesCutout3D &model);
	 ...
	 */
	class FaceElypsesCutout3D : public Layer4D
	{

		friend class _priv::FaceElypsesCutout3DImpl;

	public:
		/**
		 * @brief Construct a new FaceElypsesCutout3D object
		 *
		 */
		FaceElypsesCutout3D();

		/**
		 * @brief Construct a new FaceElypsesCutout3D object, this one allows passing a trained mdoel weights by giving the exp name.
		 *
		 */
		FaceElypsesCutout3D(size_t filter_number, size_t filter_width, size_t filter_height, size_t filter_depth = 1, std::string model_path = "", size_t stride_x = 1,
					  size_t stride_y = 1, size_t stride_k = 1, size_t padding_x = 0, size_t padding_y = 0, size_t padding_k = 0);

		FaceElypsesCutout3D(const FaceElypsesCutout3D &that) = delete;
		FaceElypsesCutout3D &operator=(const FaceElypsesCutout3D &that) = delete;
	...
	private:
		...
		_priv::FaceElypsesCutout3DImpl _impl;
